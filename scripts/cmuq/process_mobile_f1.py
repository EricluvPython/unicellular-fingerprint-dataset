"""
CMUQ Floor 1 – Mobile Processing
Converts raw Firebase JSON export of a free-walk session →
data/cmuq/mobile/floor1.csv
...
Usage:
    python scripts/cmuq/process_mobile_f1.py
"""
...
# TODO: update raw filename when floor-1 mobile export is available
JSON_PATH = os.path.join(ROOT, "raw", "fingerprints_floor1_mobile.json")
OUT_CSV   = os.path.join(ROOT, "data", "cmuq", "mobile", "floor1.csv")
FLOOR     = 1
...
def main():
    if not os.path.exists(JSON_PATH):
        ...
        print("       Place the floor-{FLOOR} mobile Firebase export in raw/ and update JSON_PATH.")
        return
    ...
    # Unwrap top-level wrapper key if present (matches floor-1 stationary format)
