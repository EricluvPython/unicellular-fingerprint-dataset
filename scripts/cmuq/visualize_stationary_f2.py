"""
Visualization of CMUQ Floor 2 Stationary Dataset
Generates: cmuq_f2_stationary_overview.png
Run AFTER process_cmuq_f2_stationary.py and assign_coordinates_f2.py.
"""
...
fig.suptitle(
    "CMUQ Floor 2 – Stationary Dataset Overview\n"
    f"84 RPs · {n_phones} Phones · 300 scans/RP/phone · "
...
short_labels = [p.replace("25028RN03A-2","RN03A-2").replace("25028RN03A","RN03A") for p in phones]
...
ax_tx.set_xticks(np.arange(1, 85, 7)); ax_tx.tick_params(axis="x", labelsize=7)
...
serving_clean = serving.copy()
snr_by_phone = [(p, serving_clean[serving_clean["phoneName"]==p]["transmitter_snr"].dropna().values)
                for p in phones]
