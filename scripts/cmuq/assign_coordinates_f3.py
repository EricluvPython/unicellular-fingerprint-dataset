"""
Interactive coordinate assignment – CMUQ Floor 3.
Identical workflow to assign_coordinates_f1.py / assign_coordinates_f2.py.

Floor 3 has two sub-areas on one plan:
  CMU-F3-1 (RPs  1-32)  – upper straight corridor
  CMU-F3-2 (RPs 33-70)  – lower curved arc
  Total: 70 RPs

Phase 1: OpenCV detects red circles automatically (cached in
         detected_circles_f3.json / detected_circles_f3.png).
Phase 2: Click circles in RP order (1 → 70).  Each click snaps to the
...
PDF_PATH        = os.path.join(ROOT, "raw", "TF-Generic(3).pdf")
...
def run_ordering(img_rgb, circles, coords):
    ...
    fig.canvas.manager.set_window_title("CMUQ Floor 3 – Coordinate Assignment")
